...
use crate::{
    data::page::{
        Page, PageDataLayout, PageError, PageFileMetadata, ReadMetadataError, Record,
        RecordIterator,
    },
    schema::{
        TableSchema,
        table::{Row, RowDeserializationError, Table},
    },
...
pub trait Store {
    ...
    fn seq_page_iterator<'db>(
        &'db self,
        layout: &'db PageDataLayout,
        page_storage: &'db dyn PageStorage,
        ...
        loop {
            if let Some(record_iter) = self.record_iter.as_mut() {
                if let Some(r) = record_iter.next() {
                    return Some(
                        Row::deserialize(r.data(), self.table.schema())
                            .map(|row| (r, row))
...
impl Iterator for PageRowIterator {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.record_iterator.next().map(|r| {
            Row::deserialize(r.data(), &self.schema)
                .map(|row| (r, row))
                .map_err(StoreError::from)
        })
    }
