...
#[derive(Debug, Error)]
pub enum FsmError {
    #[error("Invalid FSM page layout: {0}")]
    ...
    #[error("Invalid FSM traversal to parent from root level")]
    InvalidParentTraversal,
    #[error("Invalid FSM traversal to child from leaf level")]
    InvalidChildTraversal,
    #[error("Corrupted FSM state: {0}")]
    CorruptedState(String),
    #[error("FSM store error")]
    Store(#[from] StoreError),
    #[error("FSM page error")]
    Page(#[from] PageError),
}
...
impl<'db, S: Store> Fsm<'db, S> {
    ...
    pub fn update(&self, page: &Page<'_>) -> Result<(), FsmError> {
        ...
        let meta = self.store.read_metadata(self.page_layout, &access)?;
    ...
    pub fn find_available_page(&self, bytes: usize) -> Result<Page<'db>, FsmError> {
        // start at root:
        ...
        // Init
        let meta_data = self.store.read_metadata(self.page_layout, &access)?;
        ...
        for _ in 0..self.depth {
            ...
            let fsm_page = self.store.read_page(self.page_layout, page_id, &access)?;
        ...
        if heap_page_id < 1 {
            return Err(FsmError::CorruptedState("No heap page found".to_owned()));
        }
    ...
    fn parent(&self, child_addr: &FsmAddress) -> Result<(FsmAddress, usize), FsmError> {
        if child_addr.level == self.root_level {
            return Err(FsmError::InvalidParentTraversal);
        }
    ...
    fn child(&self, parent_addr: &FsmAddress, slot: usize) -> Result<FsmAddress, FsmError> {
        if parent_addr.level == 0 {
            return Err(FsmError::InvalidChildTraversal);
        }
    ...
    fn allocate_new_fsm_and_init(&self, access: &FsmAccess) -> Result<(), FsmError> {
        let mut page = self.store.allocate_page(self.page_layout, access)?;
        let binary_tree = BinaryTree::new(self.page_layout);
        page.insert_record(binary_tree.serialize())?;
        self.store.write_page(self.page_layout, &page, access)?;
        Ok(())
    }
    ...
    fn heap_page_id_to_addr(&self, page_id: i32) -> Result<(FsmAddress, usize), FsmError> {
        let slot_index_globally = page_id - 1;
        if slot_index_globally < 0 {
            return Err(FsmError::CorruptedState(format!(
                "Slot index cannot be less than 0 (page_id={page_id})"
            )));
        }
    ...
    fn addr_slot_to_heap_page_id(
        ...
        Ok(slot_index_globally as i32 + 1)
    }
    ...
    fn read_tree(&self, page: &Page<'_>) -> Result<BinaryTree, FsmError> {
        ...
        if bytes.len() != expected_len {
            return Err(FsmError::CorruptRootSlotData {
                page_id,
                expected_len,
...
mod tests {
    ...
    fn should_walk_the_tree_upwards() {
        ...
        // Slot is 2000 % 108 = 56
        let (addr, slot) = fsm.parent(&child_addr).unwrap();
        ...
        // Slot is 185 % 108 = 77
        let (addr, slot) = fsm.parent(&addr).unwrap();
    ...
    fn should_update_tree() {
        ...
        let needed_100_byte_page = fsm.find_available_page(100).unwrap();
    ...
    fn should_init_tree_once_if_first_request() {
        ...
        let page = fsm.find_available_page(0).unwrap();
    ...
    fn should_read_binary_tree_from_page() {
        ...
        let page = store.read_page(&layout, 1, &fsm_access).unwrap();
