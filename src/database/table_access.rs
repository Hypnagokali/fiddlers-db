...
use crate::{
    data::page::{Page, PageDataLayout, PageError, Record},
    freespace::fsm::{Fsm, FsmError},
    schema::{
        Column, TableSchema,
        table::{Cell, Row, RowDeserializationError, RowValidationError, Table},
    },
    store::{IndexedRowIterator, PageRowIterator, Store, StoreError},
    tree::store::{BTreeStore, BTreeStoreError},
};
...
impl UpdateIndexCommand {
    ...
    fn push_update(&mut self, cell_update: (usize, i32, i32)) {
        self.update_cells
            .push((cell_update.0, Some(cell_update.1), Some(cell_update.2)));
    }

    fn push_insert(&mut self, cell_update: (usize, i32)) {
        self.update_cells
            .push((cell_update.0, None, Some(cell_update.1)));
    }

    fn push_delete(&mut self, cell_update: (usize, i32)) {
        self.update_cells
            .push((cell_update.0, Some(cell_update.1), None));
    }
}
...
impl<'db, R: 'db> QueryResult<'db, R> {
    ...
    pub fn filter<F: Fn(&R) -> bool + 'db>(self, f: F) -> QueryResult<'db, R> {
        let iter = self.row_iter.filter_map(move |res| match res {
            Ok(record) => {
                if f(&record) {
                    Some(Ok(record))
                } else {
...
impl<'db> QueryResult<'db, (Record, Row)> {
    ...
    pub fn hash_join(
        ...
        let that_col_type = inner_query.schema.columns[that_col_index]
            .col_type
            .raw_type();

        if this_col_type != that_col_type {
            return Err(TableAccessError::LoadRowsError(format!(
                "Join columns have different types: {} vs {}",
                this_col_type, that_col_type
            )));
        }
        ...
        let joined_cols: Vec<Column> = self
            .schema
            .columns
            .iter()
...
fn find_column_for_query_by_cell(
    schema: &TableSchema,
    col_name: &str,
    cell: &Cell,
) -> Result<usize, TableAccessError> {
...
impl<'db, S: Store> TableAccess<'db, S> {
    ...
    pub fn find(
        ...
        col_name: &str,
        cell: Cell,
    ) -> Result<QueryResult<'db, (Record, Row)>, TableAccessError> {
        ...
        if let Some(btree_pointer) = col_index_map.get(&col_index) {
            let val = cell
                .expect_int("Indexed values need to be of type Int")
            ...
            let res = self.indexed_columns[*btree_pointer]
                .1
                .borrow()
                .find(val)
                .map_err(|e| TableAccessError::LoadRowsError(e.to_string()))?
            ...
            Ok(qr.filter(move |(_, row)| row.cells()[col_index] == cell))
        }
    }
    ...
    pub fn delete(&self, query_result: QueryResult<(Record, Row)>) -> Result<(), TableAccessError> {
        ...
        for (record, row) in query_result.rows()? {
            ...
            for (col_idx, btree_idx) in col_index_btree_map.iter() {
                let val = row.cells()[*col_idx]
                    .expect_int("Indexed value must be of type Int")
        ...
        for (page_id, records_to_delete) in page_row_map {
            for (record, uic) in records_to_delete {
                let mut page = self
                    .store
                    .read_page(self.layout, page_id, &self.table)
                    .map_err(|e| TableAccessError::DeleteRowsError(e.to_string()))?;
                ...
                self.store
                    .write_page(self.layout, &page, &self.table)
                    .map_err(|e| TableAccessError::DeleteRowsError(e.to_string()))?;
        ...
        for (idx, old_val, new_val) in update_index_cmd.update_cells {
            ...
            if let Some(new_val) = new_val {
                self.indexed_columns[idx]
                    .1
                    .borrow_mut()
                    .insert(new_val, (page_id, slot_id as i32))
                    .map_err(|e| TableAccessError::UpdateRowsError(e.to_string()))?;
            }
        }
        ...
        self.indexed_columns
            ...
            .map(|(btree_pointer, (col_id, _))| {
                let col_idx = self
                    .table
                    .schema()
                    .find_index_by_id(col_id)
                    .ok_or_else(|| {
                        TableAccessError::UpdateRowsError(format!(
                            "Cannot find columnd with id {} in schema",
                            col_id
                        ))
                    })?;
    ...
    pub fn update(
        &self,
        query_result: QueryResult<(Record, Row)>,
        updates: Vec<(&str, Cell)>,
    ) -> Result<(), TableAccessError> {
        ...
        let not_in_place_update = updates
            .iter()
            .any(|(_, cell)| cell.column_type().is_var_size());
        ...
        let mut updated_rows_map: HashMap<i32, Vec<(Record, Row, UpdateIndexCommand)>> =
            HashMap::new();
                ...
                if let Some(updated_cell) = new_values.get(&queried_cell_index) {
                    if let Some(btree_pointer) = index_to_btree_pointer_map.get(&queried_cell_index)
                    {
                        ...
                        let old_value = old_cell
                            .expect_int("Int expected for indexed values")
                            .map_err(|e| TableAccessError::UpdateRowsError(e.to_string()))?;
                        let new_value = updated_cell
                            .expect_int("Int expected for indexed values")
                            .map_err(|e| TableAccessError::UpdateRowsError(e.to_string()))?;
            ...
            if not_in_place_update {
                ...
                for (record, updated_row, update_index_cmd) in updated_rows {
                    ...
                    self.store
                        .write_page(self.layout, &page, &self.table)
                        .map_err(|_| {
                            TableAccessError::UpdateRowsError(
                                "Reinsert update error: cannot write page".to_string(),
                            )
                        })?;
                ...
                for (record, updated_row, update_index_cmd) in updated_rows {
                    ...
                    self.store
                        ...
                        .map_err(|_| {
                            TableAccessError::UpdateRowsError(
                                "Write in place error: cannot write page".to_string(),
                            )
                        })?;
        ...
        for (updated_row_data, update_index_cmd) in rows_needs_another_page {
            self.raw_insert(updated_row_data, move |s, (page_id, slot_id)| {
    ...
    fn raw_insert<B: FnOnce(&Self, (i32, usize)) -> Result<(), TableAccessError>>(
        &self,
        row_data: Vec<u8>,
        before_saving_hook: B,
    ) -> Result<(i32, usize), TableAccessError> {
        let fsm = self.fsm()?;
        let mut page = fsm.find_available_page(row_data.len())?;

        if !page.can_insert(&row_data) {
            return Err(TableAccessError::InsertRowError(
                "Critical: FSM returned page with too little space. Likely a corruption of FSM."
                    .to_owned(),
            ));
        }
...
mod tests {
    ...
    use crate::{
        ...
        database::table_access::TableAccess,
        freespace::fsm::Fsm,
        schema::{
            Column, ColumnType, TableSchema,
            table::{Cell, Row, Table},
        },
        store::{IndexedRowIterator, Store, file_store::FileStore},
    };
    ...
    fn should_delete_multiple() {
        ...
        let first = Row::new(vec![Cell::Varchar("Rabbit".to_owned()), Cell::Int(42)]);
        let second = Row::new(vec![Cell::Varchar("Rabbit".to_owned()), Cell::Int(52)]);
        let third = Row::new(vec![Cell::Varchar("Rabbit".to_owned()), Cell::Int(62)]);
        let fourth = Row::new(vec![Cell::Varchar("Hare".to_owned()), Cell::Int(82)]);
        ...
        let result = access
            .find("value", Cell::Varchar("Rabbit".to_owned()))
            .unwrap();
        ...
        assert_eq!(
            rows[0].1.cells(),
            &[Cell::Varchar("Hare".to_owned()), Cell::Int(82)]
        );
    }
    ...
    fn should_update_multiple_with_delete_reinsert() {
        ...
        access
            .update(result, vec![("value", Cell::Varchar("Bear".to_owned()))])
            .unwrap();
        ...
        let result = access
            .find("value", Cell::Varchar("Bear".to_owned()))
            .unwrap();
        let rows = result.rows().unwrap();
    ...
    fn should_update_with_delete_reinsert() {
        ...
        let first_row = Row::new(vec![Cell::Varchar("Rabbit".to_owned()), Cell::Int(42)]);
        let second_row = Row::new(vec![Cell::Varchar("Hare".to_owned()), Cell::Int(82)]);
    ...
    fn should_ensure_unique_values() {
        ...
        let access =
            TableAccess::new(table.clone(), &store, &layout).with_indexes(vec![(1, btree)]);

        let fsm_access = Fsm::access(&table);
        store.create(&layout, &fsm_access).unwrap();

        let first_row = Row::new(vec![Cell::Int(42), Cell::Byte(1)]);
        let second_row = Row::new(vec![Cell::Int(42), Cell::Byte(2)]);
    ...
    fn should_insert_using_index() {
        ...
        let row = page
            .read_slot(slot_id as usize)
            .map(|data| Row::deserialize(data, table.schema()))
            .unwrap()
            .unwrap();
    ...
    fn should_update_and_use_index() {
        ...
        let first_row = Row::new(vec![Cell::Int(42), Cell::Byte(1)]);
        let second_row = Row::new(vec![Cell::Int(82), Cell::Byte(2)]);
        ...
        access
            .update(result, vec![("value", Cell::Int(99))])
            .unwrap();
    ...
    fn should_insert_two_rows() {
        ...
        let first_row = Row::new(vec![Cell::Varchar("Hans".to_owned())]);
        let second_row = Row::new(vec![Cell::Varchar("Rabbit".to_owned())]);
    ...
    fn should_find_a_row() {
        ...
        let first_row = Row::new(vec![Cell::Int(1), Cell::Varchar("Hans".to_owned())]);

        let second_row = Row::new(vec![Cell::Int(2), Cell::Varchar("Rabbit".to_owned())]);

        access.insert(&first_row).unwrap();
        access.insert(&second_row).unwrap();

        let result = access
            .find("name", Cell::Varchar("Hans".to_owned()))
            .unwrap();
        let rows = result.rows().unwrap();
        assert_eq!(rows.len(), 1);
        let row = rows.get(0).unwrap();
        assert!(
            matches!(row.1.cells().as_slice(), [Cell::Int(id), Cell::Varchar(name)] if *id == 1 && name == "Hans")
        );
    }
    ...
    fn should_find_multiple_rows() {
        ...
        let second_row = Row::new(vec![Cell::Int(2), Cell::Varchar("Hans".to_owned())]);
        ...
        assert!(
            matches!(row.1.cells().as_slice(), [Cell::Int(id), Cell::Varchar(name)] if *id == 2 && name == "Hans")
        );
    }
    ...
    fn should_join_two_tables() {
        ...
        let first_person = Row::new(vec![Cell::Int(1), Cell::Varchar("Hans".to_owned())]);

        let second_person = Row::new(vec![Cell::Int(2), Cell::Varchar("Rabbit".to_owned())]);
        ...
        let first_address = Row::new(vec![Cell::Int(1), Cell::Varchar("Lilienstr 99".to_owned())]);
        ...
        let result = person_result
            .hash_join(address_result, "id", "person_id")
            .unwrap();
        assert_eq!(result.schema.columns.len(), 4);
        assert_eq!(
            result.schema.columns[0],
            Column::new(1, "id", ColumnType::Int)
        );
        assert_eq!(
            result.schema.columns[1],
            Column::new(2, "name", ColumnType::Varchar(10))
        );
        assert_eq!(
            result.schema.columns[2],
            Column::new(1, "person_id", ColumnType::Int)
        );
        assert_eq!(
            result.schema.columns[3],
            Column::new(2, "address", ColumnType::Varchar(64))
        );
